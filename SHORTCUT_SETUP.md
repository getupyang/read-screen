...
*   **动作 5**: `获取 URL 的内容` (Get Contents of URL)
    *   **URL**: `https://read-screen.vercel.app/api/trigger-simple` (请替换为你的实际域名)
    *   **方法**: **GET**
    *   **说明**: 这个请求会立即触发后台AI分析，让卡片提前生成好。

### 5. 反馈 (Feedback)

*   **动作 6**: `显示通知` (Show Notification)
    *   **标题**: ✨ 已保存
    *   **正文**: 正在后台生成卡片...
    *   **播放声音**: **关闭** (这样更安静，不打扰)。

*   **删除旧动作**: 如果你之前添加了"快速查看"，请**删除**它，否则会弹窗阻断操作。
...
1.  在小红书截图。
2.  点击分享 -> "Snapshot AI"。
3.  顶部弹出一条小通知 "✨ 已保存: 正在后台生成卡片..."。
4.  继续刷小红书（约3秒完成）。
5.  **后台自动AI分析**（10-30秒，用户无感知）。
