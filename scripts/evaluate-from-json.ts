 ...
 *
 * 用法：
 * npm run evaluate-json case_001 ai-output.json <gemini-api-key>
 *
 * 或者直接传入JSON字符串：
 * npm run evaluate-json case_001 '{"cards":[...]}' <gemini-api-key>
 */
...
const aiOutputInput = process.argv[3];
const geminiApiKey = process.argv[4];

if (!caseId || !aiOutputInput || !geminiApiKey) {
  console.error('❌ 缺少参数');
  console.log('\n用法1 (从文件读取):');
  console.log('  npm run evaluate-json <case-id> <json-file-path> <gemini-api-key>');
  console.log('\n用法2 (直接传入JSON):');
  console.log('  npm run evaluate-json <case-id> \'{"cards":[...]}\' <gemini-api-key>');
  console.log('\n示例:');
  console.log('  npm run evaluate-json case_001 output.json AIza...');
  process.exit(1);
}
...
function loadAIOutput(input: string): any {
  // 尝试作为文件路径读取
...
async function evaluateCase(): Promise<void> {
  ...
  let aiOutput;
  try {
    aiOutput = loadAIOutput(aiOutputInput);
    console.log(`✅ AI输出: ${aiOutput.cards?.length || 0} 张卡片`);
  } catch (error: any) {
    console.error(`❌ 无法加载AI输出: ${error.message}`);
    console.log(`\n💡 提示: 请检查JSON格式是否正确`);
    process.exit(1);
  }
