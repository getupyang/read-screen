 ...
 *
 * 用法：
 * npm run evaluate-remote case_001 <vercel-url> <gemini-api-key>
 *
 * 示例：
 * npm run evaluate-remote case_001 https://your-app.vercel.app AIza...
 */
...
const geminiApiKey = process.argv[4];

if (!caseId || !vercelUrl || !geminiApiKey) {
  console.error('❌ 缺少参数');
  console.log('\n用法：');
  console.log('  npm run evaluate-remote <case-id> <vercel-url> <gemini-api-key>');
  console.log('\n示例：');
  console.log('  npm run evaluate-remote case_001 https://your-app.vercel.app AIza...');
  process.exit(1);
}
...
async function getInboxData(vercelUrl: string): Promise<any[]> {
  const apiUrl = `${vercelUrl}/api/inbox-data`;

  console.log(`🔍 从Vercel API获取数据: ${apiUrl}`);
...
async function findInboxItem(vercelUrl: string, imageUrl: string, filename: string): Promise<any> {
  const items = await getInboxData(vercelUrl);

  // 先尝试精确匹配
  let item = items.find(i => i.image_url === imageUrl);

  // 如果没找到，尝试文件名模糊匹配
  if (!item) {
    item = items.find(i => i.image_url.includes(filename));
  }
...
async function evaluateCase(): Promise<void> {
  ...
  let inboxItem;
  try {
    inboxItem = await findInboxItem(
      vercelUrl,
      testCase.image.supabase_url,
      testCase.image.filename
    );
  } catch (error: any) {
    console.error(`❌ 无法连接到Vercel API: ${error.message}`);
    console.log(`\n💡 提示: 请检查Vercel URL是否正确`);
    console.log(`   URL: ${vercelUrl}`);
    process.exit(1);
  }

  if (!inboxItem) {
    console.error(`❌ 在Inbox中找不到该图片的记录`);
    console.log(`   图片URL: ${testCase.image.supabase_url}`);
    console.log(`   文件名: ${testCase.image.filename}`);
    console.log(`\n💡 提示: 请先上传该图片并等待分析完成`);
    process.exit(1);
  }
...
evaluateCase().catch(console.error);
